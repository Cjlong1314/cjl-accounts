...
import { isWithinRetention, RETENTION_YEARS, retentionCutoffIso } from './retention'
import { DEFAULT_CATEGORY_ICON, normalizeCategoryIcon } from './categoryIcons'

export type LedgerFileName = 'categories' | 'transactions'

export interface LedgerFileBackend {
...
export class MarkdownStore {
  ...
  constructor(private readonly backend: LedgerFileBackend) {
    this.ensureFiles()
  }

  get dataDir(): string {
    return this.backend.dataDir
  }

  private ensureFiles(): void {
    this.backend.ensureReady()
    if (!this.backend.exists('categories')) {
      this.writeCategories(defaultCategories())
    } else {
      this.migrateCategoryIcons()
    }
    if (!this.backend.exists('transactions')) {
  ...
  private run<T>(fn: () => T): Promise<T> {
    const result = this.queue.then(async () => {
      const value = fn()
      if (this.backend.flush) await this.backend.flush()
      return value
    })
  ...
  private readRows(name: LedgerFileName): Row[] {
    return parseMarkdownTable(this.backend.read(name)).rows
  }

  private writeAtomic(name: LedgerFileName, content: string): void {
    this.backend.write(name, content)
  }
  ...
  private writeCategories(categories: Category[]): void {
    ...
    this.writeAtomic('categories', serializeMarkdownTable('分类', ['id', 'name', 'kind', 'icon'], rows))
  }
